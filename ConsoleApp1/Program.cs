...
namespace DogShow
{
    public class Program
    {
        ...
        static void AddDogMenu(ShowOrganizer organizer)
        {
            ...
            if (string.IsNullOrWhiteSpace(organizer.Club))
            {
                organizer.Club = dog.Club;
            }
        ...
        public class ShowOrganizer
        {
            ...
            public void AddDog(Dog dog)
            {
                ...
                // Если в ShowOrganizer.Club ещё пусто, установим клуб из собаки
                if (string.IsNullOrWhiteSpace(this.Club) && !string.IsNullOrWhiteSpace(dog.Club))
                {
                    this.Club = dog.Club;
                }
            ...
            private void SaveClub()
            {
                ...
                sb.AppendLine($"Клуб: {Club}");
                sb.AppendLine();

                var dogsInClub = Dogs.Where(d => d.Club.Equals(Club, StringComparison.OrdinalIgnoreCase)).ToList();
                sb.AppendLine("Собаки клуба:");

                if (dogsInClub.Count == 0)
                {
                    sb.AppendLine("Собак нет.");
                }
                else
                {
                    foreach (var dog in dogsInClub)
                    {
                        sb.AppendLine($"ID: {dog.Id}, Кличка: {dog.Name}, Порода: {dog.Breed}, Возраст: {dog.Age}, Владелец: {dog.OwnerFullName}, Медаль: {dog.Medal}");
                    }
                }

                sb.AppendLine();
                var expertsInClub = Experts.Where(e => e.Club.Equals(Club, StringComparison.OrdinalIgnoreCase)).ToList();
                sb.AppendLine("Эксперты клуба:");

                if (expertsInClub.Count == 0)
                {
                    sb.AppendLine("Экспертов нет.");
                }
                else
                {
                    foreach (var expert in expertsInClub)
                    {
                        sb.AppendLine($"ID: {expert.Id}, ФИО: {expert.FullName}, Специализация: {expert.Specialization}");
                    }
                }

                File.WriteAllText(ClubFile, sb.ToString(), Encoding.UTF8);
            }
            ...
            private void LoadClub()
            {
                ...
                var lines = File.ReadAllLines(ClubFile, Encoding.UTF8);
                foreach (var line in lines)
                {
                    if (line.StartsWith("Клуб: "))
                    {
                        Club = line.Substring("Клуб: ".Length).Trim();
                        return;
                    }
                }
                // Если не нашли строку с названием клуба
                Club = "";
            }
